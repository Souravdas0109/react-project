...
function* getTrendingMovies() {
  const records = yield fetch(request.fetchTrending).then(response =>
    response.json()
  );
...
function* getTopRatedMovies() {
  const records = yield fetch(request.fetchTopRated).then(response =>
    response.json()
  );
...
function* getPopular() {
  const records = yield fetch(request.fetchPopular).then(response =>
    response.json()
  );
...
function* searchMovies(obj) {
  const records = yield fetch(
    `${baseURL}/search/movie?api_key=${API_KEY}&query=${obj.payload}&language=en-US&sort_by=popularity.desc&include_adult=false`
